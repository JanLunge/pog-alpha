<template>
  ...
  <p class="max-w-md p-4">define the size of your keyboard matrix here, set it as big as you need. For easier wiring set it to the max number of cols/rows on your keyboard</p>
...
<div class="btn btn-primary btn-sm mb-4" @click="$emit('next')" :class="{'btn-disabled': matrixEmpty}">Next</div>
    <div class="grid-visualizer">
      <div class="row" v-for="row in selectedkeyboard.layoutContents.matrix.rows">
        <div class="col" v-for="col in selectedkeyboard.layoutContents.matrix.cols"></div>
      </div>
    </div>
</template>
...
const matrixEmpty = computed(()=>{
  return !(selectedkeyboard.value.layoutContents.matrix.cols !== 0 && selectedkeyboard.value.layoutContents.matrix.rows)
})
</script>
